...
function Cashier(name, productsDatabase) {
  ...
  this.countTotalPrice = function countTotalPrice(order){
      for (let key in order) {
          this.totalPrice = this.totalPrice + order[key] * this.productsDatabase[key];
    }
    //console.log(`Посчитает сколько нужно денег ${this.totalPrice}`);
  };

  this.getCustomerMoney = function getCustomerMoney(value){
    this.customerMoney = value;
    //console.log(`Возьмет сумму денег у покупателя ${this.customerMoney}`);
  };
  ...
  this.countChange = function countChange(){
    if(this.customerMoney >= this.totalPrice){
      ...
      //console.log(`Посчитает сдачу ${this.changeAmount}`);
    }
    return null
  };
