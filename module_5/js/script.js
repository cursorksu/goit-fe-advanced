...
console.log(`=====Голубоглазые пользователи=====`);
/**
...
console.log(`=====Эти давно не заходили=====`);
/**
...
console.log(`=====Считаем баланс=====`);
/**
 * Получить общую сумму баланса (поле balance) всех пользователей
 */
const getTotalBalance = arr =>
  arr.map(user => user.balance).reduce((acc, value) => acc + value, 0);

console.log(getTotalBalance(users)); // 20916

console.log(`=====Фмльтруем по друзьям=====`);
...
console.log(`=====ЗАДАНИЕ ПОВЫШЕННОЙ СЛОЖНОСТИ 1=====`);
/**
...
const getAllSkills = arr => {
  const arrSkills = [];
  arr.map(user => user.skills).map(skill => {
    skill.forEach(element => {
      if (arrSkills.includes(element) === false) {
        arrSkills.push(element);
      }
    });
  });
  return arrSkills.sort();
};

console.log(getAllSkills(users));
// [ 'adipisicing', 'amet', 'anim', 'commodo', 'culpa', 'elit', 'ex', 'ipsum', 'irure', 'laborum', 'lorem', 'mollit', 'non', 'nostrud', 'nulla', 'proident', 'tempor', 'velit', 'veniam' ]

console.log(`=====ЗАДАНИЕ ПОВЫШЕННОЙ СЛОЖНОСТИ 2=====`);
/**
 * Массив имен (поле name) людей, отсортированных в зависимости от количества их друзей (поле friends)
 */
const getUserNamesSortedByFriendsCount = arr =>
  arr.sort(user => user.friends.length).map(user => user.name);
